...
// УБРАН СЕРЫЙ ФОН - теперь прозрачный
...
renderer3d.setClearColor(0xffffff, 0); // Прозрачный фон
...
// --- Функция для добавления отсутствующих элементов управления ---
function ensureUIControls() {
    ...
    if (!document.getElementById('intensity')) {
        ...
        row.innerHTML = `
             ...
             <span class="value-display" id="intensityVal">1.00</span>
        `;
...
const uniforms = {
    ...
    uTileEnabled: { value: 1 }, // ВКЛЮЧЕНО для бесшовности
    uOverlayTexture: { value: null }, uUseOverlay: { value: 1 },
    ...
    uExportMode: { value: 0 } // 0: Color, 1: Normal, 2: Roughness, 3: Metallic, 4: Height, 5: AO
};
...
// ИСПРАВЛЕННЫЙ ШЕЙДЕР - убран #extension, добавлена бесшовность через fract()
...
float computePattern(vec2 uv) {
    ...
    // БЕСШОВНОСТЬ: применяем fract() для тайлинга
...
void main() {
    ...
    else if (uExportMode == 2) { // ROUGHNESS MAP
        finalColor = vec3(patternValue);
    } 
    else if (uExportMode == 3) { // METALLIC MAP
        finalColor = vec3(patternValue);
    } 
    else if (uExportMode == 4) { // HEIGHT MAP
        finalColor = vec3(patternValue);
    } 
...
function updateMaterial() {
    ...
    if (currentMesh3d) {
        if (customModel) {
            customModel.traverse(c => { if (c.isMesh) c.material = currentMaterial; });
        } else {
            currentMesh3d.material = currentMaterial;
        }
    }
}
...
function updateUniformsFromUI() {
    ...
    // УБРАН МАСШТАБ 3D - теперь используется тот же масштаб что и для 2D
    uniforms.uTile3DScale.value = uniforms.uScale.value;

    const metallicVal = document.getElementById('metallic') ? parseFloat(document.getElementById('metallic').value).toFixed(2) : '0.50';
...
function getCurrentPreset() {
    return { 
        ...
        uniforms: { 
            scale:uniforms.uScale.value, octaves:uniforms.uOctaves.value, persistence:uniforms.uPersistence.value, 
            lacunarity:uniforms.uLacunarity.value, saturation:uniforms.uSaturation.value, blendMode:uniforms.uBlendMode.value, 
            rotation:uniforms.uRotation.value, offsetX:uniforms.uOffset.value.x, offsetY:uniforms.uOffset.value.y, 
            mirror:uniforms.uMirror.value, warpEnable:uniforms.uWarpEnable.value, warpStrength:uniforms.uWarpStrength.value, 
            warpOctaves:uniforms.uWarpOctaves.value, reliefStrength:uniforms.uReliefStrength.value, 
...
savePresetBtn.onclick = () => { 
    ...
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `preset_${Date.now()}.json`; a.click(); 
};
loadPresetBtn.onclick = () => loadPresetInput.click();
loadPresetInput.onchange = e => { 
    const f = e.target.files[0]; if(!f) return; 
    const r = new FileReader(); 
    r.onload = ev => { try { applyPreset(JSON.parse(ev.target.result)); } catch(e) { alert('Ошибка'); } }; 
    r.readAsText(f); 
};
...
function update3dModel() {
    ...
    if(customModel) { 
        ...
        const m = new THREE.Mesh(createGeometry(currentGeometryType), currentMaterial); 
        scene3d.add(m); 
        currentMesh3d = m; 
    }
...
// ---- Генерация оверлея (ИСПРАВЛЕНА ЛОГИКА СЛОЁВ) ----
async function generateOverlayTexture() {
    ...
    const syncScale = uniforms.uScale.value; // Масштаб паттерна для синхронизации
    ...
    for (const layer of layers) {
        ...
        let layerScale = layer.scale;
        let tileX = Math.max(1, layer.tileX || 1);
        let tileY = Math.max(1, layer.tileY || 1);
        
        if (layer.syncWithPattern) {
            rot = syncRotation;
            offX = 0.5 + syncOffsetX;
            offY = 0.5 + syncOffsetY;
            mirror = syncMirror;
            // При синхронизации масштаб и количество паттернов зависят от масштаба генерации
            layerScale = 1.0 / syncScale;
            tileX = Math.max(1, Math.round(syncScale * 2));
            tileY = Math.max(1, Math.round(syncScale * 2));
        }

        ctx.globalAlpha = layer.opacity; // Применяем прозрачность слоя
        ...
        if (tileX === 1 && tileY === 1) {
            ...
            const tileW = imgW;
            const tileH = imgH;
            const neededX = Math.min(20, Math.ceil((size / layerScale) / tileW) + 2);
            const neededY = Math.min(20, Math.ceil((size / layerScale) / tileH) + 2);
            const startTX = -Math.floor(neededX / 2);
            const startTY = -Math.floor(neededY / 2);
            for (let ty = 0; ty < neededY; ty++) {
                for (let tx = 0; tx < neededX; tx++) {
                    const cx = (startTX + tx) * tileW;
                    const cy = (startTY + ty) * tileH;
...
function updateLayersUI() {
    ...
    layers.forEach(layer => {
        ...
        syncCheck.addEventListener('change', (e) => { 
            layer.syncWithPattern = e.target.checked; 
            generateOverlayTexture(); 
            updateLayersUI(); // Обновляем UI чтобы показать/скрыть контролы
        });
        syncRow.appendChild(syncCheck);
        syncRow.appendChild(document.createTextNode('Синхр. с ген.'));
        extraDiv.appendChild(syncRow);
        
        // Контролы тайлинга (скрыты при синхронизации)
        if (!layer.syncWithPattern) {
    ...
    if (selectedLayerId && layerOpacitySlider) {
        ...
        if (layer) {
            ...
            if (layerOpacityVal) layerOpacityVal.innerText = layer.opacity.toFixed(2);
        }
    }
}
...
// ---- Hit-test для перетаскивания слоёв ----
function findLayerUnderPointer(uv) {
    for (let i = layers.length - 1; i >= 0; i--) {
        const l = layers[i];
        if (l.tileX > 1 || l.tileY > 1 || l.syncWithPattern) {
            if (selectedLayerId === l.id) return l;
            continue;
        }
        let dx = uv.x - l.x;
        let dy = uv.y - l.y;
...
canvas2dElem.addEventListener('mousedown', (e) => {
    ...
    if (layer) {
        ...
        dragStart = { x: uv.x, y: uv.y, layerX: layer.x, layerY: layer.y };
...
canvas2dElem.addEventListener('wheel', (e) => {
    ...
    if (e.ctrlKey) {
        if (!layer.syncWithPattern) layer.rotation = (layer.rotation + delta * 20) % 360;
    } else {
...
document.getElementById('export2DBtn')?.addEventListener('click', async () => {
    ...
    if (format === 'pbr') {
        ...
        exportCanvas.width = res; exportCanvas.height = res;
...
document.getElementById('exportModelBtn')?.addEventListener('click', async () => {
    ...
    try {
        ...
        else { const g = createGeometry(currentGeometryType); exportScene.add(new THREE.Mesh(g, mat)); }
        
        if (format === 'glb') {
            new GLTFExporter().parse(exportScene, result => { const blob = new Blob([result], {type:'application/octet-stream'}); downloadBlob(blob, 'model.glb'); }, {binary:true});
        } else if (format === 'gltf') {
            new GLTFExporter().parse(exportScene, result => { const blob = new Blob([JSON.stringify(result)], {type:'application/json'}); downloadBlob(blob, 'model.gltf'); });
        } else if (format === 'obj') {
...
// --- Безопасное клонирование униформ (сохраняет ссылки на текстуры) ---
...
// --- Исправленная функция рендера PBR карт ---

...
void main() {
  ...
  if (uExportMode == 0) {
    ...
    float hR = computePattern(vUvRaw + vec2(texel.x, 0.0));
    float hD = computePattern(vUvRaw - vec2(0.0, texel.y));
    float hU = computePattern(vUvRaw + vec2(0.0, texel.y));
...
// --- Аккордеон: на десктопе отключён, на мобильных работает ---

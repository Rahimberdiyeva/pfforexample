...
const uniforms = {
    ...
    uTileEnabled: { value: 1 }, // ВКЛЮЧЕНО ПО УМОЛЧАНИЮ для бесшовности
...
float computePattern(vec2 uv) {
    ...
    // === ИСПРАВЛЕНИЕ БЕСШОВНОСТИ ===
    // Сначала масштабируем координаты, затем применяем fract для зацикливания.
    // Это гарантирует, что паттерн будет идеально стыковаться на границах (0 и 1).
    vec2 st = uv * uScale;
    if (uTileEnabled == 1) {
        st = fract(st);
    }
...
function updateUniformsFromUI() {
    ...
    uniforms.uTile3DScale.value = uniforms.uScale.value; // Синхронизация масштаба 3D с 2D
...
function getCurrentPreset() {
    return { colors: activeColors.map(c => c.getHexString()), uniforms: { scale:uniforms.uScale.value, octaves:uniforms.uOctaves.value, persistence:uniforms.uPersistence.value, lacunarity:uniforms.uLacunarity.value, saturation:uniforms.uSaturation.value, blendMode:uniforms.uBlendMode.value, rotation:uniforms.uRotation.value, offsetX:uniforms.uOffset.value.x, offsetY:uniforms.uOffset.value.y, mirror:uniforms.uMirror.value, warpEnable:uniforms.uWarpEnable.value, warpStrength:uniforms.uWarpStrength.value, warpOctaves:uniforms.uWarpOctaves.value, reliefStrength:uniforms.uReliefStrength.value, intensity:uniforms.uIntensity.value }, patternType:uniforms.uPatternType.value };
}
...
async function generateOverlayTexture() {
    ...
    const syncRotation = uniforms.uRotation.value, syncOffsetX = uniforms.uOffset.value.x, syncOffsetY = uniforms.uOffset.value.y, syncMirror = uniforms.uMirror.value, syncScale = uniforms.uScale.value;
    for (const layer of layers) {
        ctx.save();
        let rot = layer.rotation, offX = layer.x, offY = layer.y, mirror = layer.mirror || 0, layerScale = layer.scale;
        ...
        if (layer.syncWithPattern) {
            ...
            layerScale = 1.0 / syncScale; tileX = Math.max(1, Math.round(syncScale * 2)); tileY = Math.max(1, Math.round(syncScale * 2));
        }
        ...
        if (tileX === 1 && tileY === 1) ctx.drawImage(img, -imgW / 2, -imgH / 2, imgW, imgH);
            ...
            const neededX = Math.min(20, Math.ceil((size / layerScale) / imgW) + 2), neededY = Math.min(20, Math.ceil((size / layerScale) / imgH) + 2);
            const startTX = -Math.floor(neededX / 2), startTY = -Math.floor(neededY / 2);
            for (let ty = 0; ty < neededY; ty++) for (let tx = 0; tx < neededX; tx++) ctx.drawImage(img, (startTX + tx) * imgW, (startTY + ty) * imgH, imgW, imgH);
        }
        ctx.restore();
    }
...
function drawImageCover(ctx, img, w, h) {
    ...
    ctx.drawImage(img, 0, 0, img.width, img.height, (w - img.width * ratio) / 2, (h - img.height * ratio) / 2, img.width * ratio, img.height * ratio);
}
...
function updateLayersUI() {
    ...
    layers.forEach(layer => {
        ...
        syncCheck.addEventListener('change', (e) => { layer.syncWithPattern = e.target.checked; generateOverlayTexture(); updateLayersUI(); });
...
function findLayerUnderPointer(uv) {
    for (let i = layers.length - 1; i >= 0; i--) {
        const l = layers[i];
        if (l.tileX > 1 || l.tileY > 1 || l.syncWithPattern) { if (selectedLayerId === l.id) return l; continue; }
...
canvas2dElem.addEventListener('mousedown', (e) => {
    ...
    if (layer) { selectLayer(layer.id); isDraggingLayer = true; dragStart = { x: uv.x, y: uv.y, layerX: layer.x, layerY: layer.y }; canvas2dElem.style.cursor = 'grabbing'; }
...
window.addEventListener('mousemove', (e) => {
    ...
    if (layer) { layer.x = Math.min(Math.max(0, dragStart.layerX + (uv.x - dragStart.x)), 1); layer.y = Math.min(Math.max(0, dragStart.layerY + (uv.y - dragStart.y)), 1); generateOverlayTexture(); updateLayersUI(); }
});
...
canvas2dElem.addEventListener('wheel', (e) => {
    ...
    else { layer.scale = Math.min(Math.max(0.1, layer.scale + delta), 2.0); }
    generateOverlayTexture(); updateLayersUI();
});
...
async function renderPBRMap(res, type) {
    ...
    renderer.dispose(); mat.dispose(); return blob;
}
...
document.getElementById('menuToggle')?.addEventListener('click', (e) => { e.stopPropagation(); document.querySelector('.pattern-bar')?.classList.toggle('open'); });
document.addEventListener('click', (e) => {
    const menu = document.querySelector('.pattern-bar'), toggle = document.getElementById('menuToggle');
    if (menu && menu.classList.contains('open') && !menu.contains(e.target) && !toggle?.contains(e.target)) menu.classList.remove('open');
});

function animate() { requestAnimationFrame(animate); renderer2d.render(scene2d, camera2d); controls3d.update(); renderer3d.render(scene3d, camera3d); }
animate(); update3dModel(); generateOverlayTexture();

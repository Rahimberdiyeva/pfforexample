...
void main() {
  ...
  /* FIX: PBR экспорт. Normal map заменён на заглушку (0.5, 0.5, 1.0), так как экранная производная не даёт корректного tangent space.
     Рекомендуется использовать Height map для автогенерации нормали в Blender. */
  if (uExportMode == 1) finalColor = vec3(0.5, 0.5, 1.0); 
...
document.getElementById('exportModelBtn')?.addEventListener('click', async () => {
  ...
  try {
    ...
    /* FIX: Явно задаём повторение текстуры, чтобы в Blender UV не растягивались */
    ...
    /* FIX: glTF экспорт. Убран двойной JSON.stringify, так как GLTFExporter возвращает готовую строку */
    else if (format === 'gltf') new GLTFExporter().parse(exportScene, (result) => {
...
function openMenu() { patternBar.classList.add('open'); menuOverlay.classList.add('active'); }
function closeMenu() { patternBar.classList.remove('open'); menuOverlay.classList.remove('active'); }

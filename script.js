...
// Нейтральный серый фон для корректной оценки PBR материалов
scene3d.background = new THREE.Color(0x888888); 
...
// --- Освещение (Профессиональная трехточечная схема для PBR) ---
...
function ensureUIControls() {
    ...
    if (!document.getElementById('tile3dScale')) {
        ...
        row.className = 'control-row';
        row.innerHTML = `
             <label>Масштаб 3D</label>
             <input type="range" id="tile3dScale" min="0.2" max="5" step="0.02" value="1.0">
             <span class="value-display" id="tile3dScaleVal">1.00</span>
        `;
        genGroup.appendChild(row);
...
uniform float uTime;
uniform int uExportMode; // 0:Base, 1:Normal, 2:Roughness, 3:Metallic, 4:Height, 5:AO
...
void main() {
    ...
    if (uExportMode == 1) { // NORMAL MAP
        ...
        finalColor = normal * 0.5 + 0.5; // Преобразование из [-1,1] в [0,1]
    } 
...
function getCurrentPreset() {
    return { 
        ...
        uniforms: { 
            ...
            intensity:uniforms.uIntensity.value, tile3dScale:uniforms.uTile3DScale.value 
        }, 
...
async function renderPBRMap(res, type) {
    ...
    // Отключаем лишние эффекты при экспорте сырых PBR-карт
...
async function captureTextureImage() {
    const tuni = cloneUniforms(uniforms);
    tuni.uShowRelief = { value: 0 };
    tuni.uUseOverlay = { value: 0 }; // Чистый базовый цвет для экспорта модели

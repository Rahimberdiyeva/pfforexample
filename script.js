...
// ---- Функции для запекания PBR карт при экспорте 3D ----
...
document.getElementById('exportModelBtn').addEventListener('click', async () => {
    ...
    try {
        // Запекаем все необходимые текстуры
        ...
        // Загружаем все текстуры как Image для CanvasTexture
        const loadTexture = (blob) => new Promise((resolve) => {
            const img = new Image(); img.onload = () => resolve(new THREE.CanvasTexture(img)); img.src = URL.createObjectURL(blob);
        });
        ...
        if(customModel) {
            ...
            exportScene.add(new THREE.Mesh(createGeometry(currentGeometryType), material));
        }
        
        if(format === 'glb') {
            new GLTFExporter().parse(exportScene, result => {
                // result может быть ArrayBuffer для бинарного glTF
            ...
            new GLTFExporter().parse(exportScene, result => {
                // для текстового glTF result — строка или объект
                let jsonStr = (typeof result === 'string') ? result : JSON.stringify(result, null, 2);

...
// --- Функция для добавления отсутствующих элементов управления (исправлена, без :has) ---
function ensureUIControls() {
    const genGroup = document.querySelector('.setting-group');
    if (!genGroup) return;

    // Добавляем интенсивность, если её нет
    ...
    // Добавляем масштаб 3D, если его нет
...
function createMaterial() {
    ...
    // Проверка компиляции шейдера (при возникновении ошибки выводим в консоль)
    const gl = renderer2d.getContext();
    if (mat.program && gl) {
        if (!gl.getProgramParameter(mat.program, gl.LINK_STATUS)) {
            console.error('Ошибка линковки шейдера:', gl.getProgramInfoLog(mat.program));
        }
    } else {
        console.warn('Шейдерная программа не создана — возможна ошибка GLSL');
    }
...
// ---- Обновление параметров генерации (с проверками на существование элементов) ----
...
// Подключаем слушатели ко всем существующим контролам
...
// Добавляем недостающие элементы управления (Intensity, Tile3DScale) и затем обновляем
ensureUIControls();
...
document.getElementById('modelFileInput')?.addEventListener('change', e => {
    ...
    new GLTFLoader().load(url, gltf => {
        ...
        update3dModel(); URL.revokeObjectURL(url); document.getElementById('modelStatus').textContent='Модель загружена'; setTimeout(()=>document.getElementById('modelStatus').textContent='',2000);
...
// ---- Генерация оверлея (исправлена синхронизация трансформаций) ----
async function generateOverlayTexture() {
    ...
    for (const layer of layers) {
        ...
        if (layer.syncWithPattern) {
            rot = syncRotation;
            // В шейдере offset применяется к UV (диапазон -1..1) после поворота, поэтому переводим в пиксели умножением на size
            offX = syncOffsetX;
            offY = syncOffsetY;
            mirror = syncMirror;
        }
        ...
        if (layer.syncWithPattern) {
            // Для синхронизации сдвиг в пикселях: offset * size (без вычитания 0.5)
            ctx.translate(offX * size, offY * size);
        } else {
            // Для обычных слоёв координаты (0..1) переводим в систему с центром в (0.5,0.5)
...
async function captureTextureImage() {
    const tuni = JSON.parse(JSON.stringify(uniforms)); tuni.uShowRelief = { value:0 };
    const sc = new THREE.Scene(); const cam = new THREE.OrthographicCamera(-1,1,1,-1,0.1,10); cam.position.z=1;
    const mat = new THREE.ShaderMaterial({ uniforms: tuni, vertexShader, fragmentShader });
    sc.add(new THREE.Mesh(new THREE.PlaneGeometry(2,2), mat));
    const renderer = new THREE.WebGLRenderer({ preserveDrawingBuffer: true });

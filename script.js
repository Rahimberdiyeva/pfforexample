...
void main() {
  ...
  float h1 = computePattern(vUv + texel);
  float h2 = computePattern(vUv + vec2(-texel.x, texel.y));
  float h3 = computePattern(vUv + vec2(texel.x, -texel.y));
  float h4 = computePattern(vUv - texel);
...
function updateUniformsFromUI() {
  ...
  const vals = {
    ...
    reliefStrengthVal: uniforms.uReliefStrength.value.toFixed(2), intensityVal: uniforms.uIntensity.value.toFixed(2),
    normalStrengthVal: uniforms.uNormalStrength.value.toFixed(2), roughnessContrastVal: uniforms.uRoughnessContrast.value.toFixed(2),
    metalThresholdVal: uniforms.uMetalThreshold.value.toFixed(2), metalScaleVal: uniforms.uMetalScale.value.toFixed(2)
  };
...
function update3dModel() {
  ...
  if(customModel) {
    ...
    currentMesh3d = new THREE.Mesh(createGeometry(currentGeometryType), material);
...
// --- Интеграция (только Blender и Unity) ---

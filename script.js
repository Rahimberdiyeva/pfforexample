...
const scene3d = new THREE.Scene(); scene3d.background = new THREE.Color(0x111122);
const camera3d = new THREE.PerspectiveCamera(45, 1, 0.1, 1000); camera3d.position.set(2, 1.5, 2.5);
...
let activeColors = [
  new THREE.Color('#FF6B8B'), new THREE.Color('#4CC9F0'),
  new THREE.Color('#F9C74F'), new THREE.Color('#9B5DE5')
];
let layers = [], selectedLayerId = null, isDraggingLayer = false, dragStart = {};
...
const fragmentShader = `
  ...
  float worley(vec2 uv) {
    ...
    for(int j=-1; j<=1; j++) for(int i=-1; i<=1; i++) { vec2 b = vec2(float(i), float(j)); vec2 r = b - f + random(p + b); res = min(res, dot(r,r)); }
    return sqrt(res);
  }
  ...
  vec2 domainWarp(vec2 uv, float strength, int octaves) {
    vec2 warped = uv;
    for(int i=0; i<5; i++) { if(i >= octaves) break; warped += strength * vec2(sin(warped.y * 3.14159 * 2.0 * float(i+1)), cos(warped.x * 3.14159 * 2.0 * float(i+1))); }
    return warped;
  }
  ...
  float wfcPattern(vec2 uv) {
    vec2 tile = floor(uv * 8.0); float hashVal = random(tile); int rule = int(floor(hashVal * 6.0));
...
scene2d.add(plane2d);
const defaultMesh = new THREE.Mesh(new THREE.BoxGeometry(1.5,1.5,1.5), material);
...
function renderAll() { renderer2d.render(scene2d, camera2d); renderer3d.render(scene3d, camera3d); }
...
function updateSizes() {
  ...
  if (w && h) { renderer3d.setSize(w, h); camera3d.aspect = w / h; camera3d.updateProjectionMatrix(); renderAll(); }
}
...
function update3dModel() {
  ...
  if (customModel) {
    ...
    scene3d.add(customModel); currentMesh3d = customModel;
  } else {
    let geom;
...
const geometricPatterns = [{name:"Шахматная",v:8},{name:"Полосы",v:9},{name:"Круги",v:10},{name:"Сетка",v:11},{name:"Плитка",v:16},{name:"Древесина",v:14},{name:"Мрамор",v:15},{name:"Truchet",v:3}];
...
document.getElementById('lightIntensity')?.addEventListener('input', e => { updateLightIntensity(parseFloat(e.target.value)); document.getElementById('lightIntensityVal').innerText = parseFloat(e.target.value).toFixed(2); renderAll(); });
...
function rebuildColorUI() {
  ...
  activeColors.forEach((col, idx) => {
    ...
    colorInput.addEventListener('input', e => { activeColors[idx] = new THREE.Color(e.target.value); updateColorUniforms(); renderAll(); });
    div.appendChild(colorInput);
    if (activeColors.length > 2) {
      const removeBtn = document.createElement('button'); removeBtn.textContent = '✕'; removeBtn.className = 'remove-color-btn';
      removeBtn.addEventListener('click', e => { e.stopPropagation(); activeColors.splice(idx,1); rebuildColorUI(); updateColorUniforms(); renderAll(); });
      div.appendChild(removeBtn);
    }
    colorContainer.appendChild(div);
  });
}
addColorBtn.addEventListener('click', () => { if (activeColors.length < 8) { activeColors.push(new THREE.Color('#FFB347')); rebuildColorUI(); updateColorUniforms(); renderAll(); } });
rebuildColorUI();
...
async function generateOverlayTexture() {
  ...
  for (const layer of layers) {
    ...
    const img = layer.imgElement, tileX = Math.max(1, layer.tileX || 1), tileY = Math.max(1, layer.tileY || 1);
    if (tileX === 1 && tileY === 1) ctx.drawImage(img, -img.width/2, -img.height/2, img.width, img.height);
    else {
...
// --- Слои UI (сокращённо, но функционально) ---
...
function updateLayersUI() {
  ...
  layers.forEach(layer => {
    ...
    syncCheck.addEventListener('change', () => { layer.syncWithPattern = syncCheck.checked; generateOverlayTexture(); });
  ...
  if (selectedLayerId && layerOpacitySlider) {
    ...
    if (layer) { layerOpacitySlider.value = layer.opacity; layerOpacityVal.innerText = layer.opacity.toFixed(2); }
  }
}
function selectLayer(id) { selectedLayerId = id; updateLayersUI(); renderAll(); }
function deleteLayerById(id) {
  const idx = layers.findIndex(l => l.id === id);
  if (idx !== -1) { layers.splice(idx,1); if (selectedLayerId === id) selectedLayerId = null; generateOverlayTexture(); updateLayersUI(); renderAll(); }
}
document.getElementById('clearOverlayBtn')?.addEventListener('click', () => { layers = []; selectedLayerId = null; generateOverlayTexture(); updateLayersUI(); renderAll(); });
if (layerOpacitySlider) {
  layerOpacitySlider.addEventListener('input', () => {
    if (selectedLayerId) {
      const layer = layers.find(l => l.id === selectedLayerId);
      if (layer) { layer.opacity = parseFloat(layerOpacitySlider.value); layerOpacityVal.innerText = layer.opacity.toFixed(2); generateOverlayTexture(); updateLayersUI(); renderAll(); }
    }
  });
}
async function loadFilesAsLayers(files) {
  ...
  generateOverlayTexture(); updateLayersUI(); renderAll();
}
...
canvas2dElem.addEventListener('drop', async e => { e.preventDefault(); canvas2dElem.style.border = 'none'; const files = Array.from(e.dataTransfer.files).filter(f => f.type.startsWith('image/')); if (files.length) await loadFilesAsLayers(files); renderAll(); });
...
window.addEventListener('mousemove', e => {
  ...
  if (layer) {
    ...
    layer.x = newX; layer.y = newY; generateOverlayTexture(); updateLayersUI(); renderAll();
  }
});
window.addEventListener('mouseup', () => { isDraggingLayer = false; canvas2dElem.style.cursor = 'crosshair'; renderAll(); });
...
// --- Экспорт 2D и 3D (полные функции) ---
...
document.getElementById('exportModelBtn')?.addEventListener('click', async () => {
  ...
  try {
    ...
    if (format === 'glb') exporter.parse(exportScene, result => downloadBlob(new Blob([result], {type:'application/octet-stream'}), 'model.glb'), {binary:true});
    else if (format === 'gltf') exporter.parse(exportScene, result => downloadBlob(new Blob([JSON.stringify(result, null, 2)], {type:'application/json'}), 'model.gltf'), {binary:false});
    else if (format === 'obj') {
...
function cloneUniforms(src) { const dst = {}; for (const k in src) dst[k] = { value: src[k].value }; return dst; }
...
async function updatePBRPreviews() {
  ...
  if (pbrGrid.children.length === 0) {
    ...
    maps.forEach((type,i) => { const div = document.createElement('div'); div.className='pbr-item'; const canvas = document.createElement('canvas'); canvas.width=128; canvas.height=128; const span = document.createElement('span'); span.textContent=names[i]; div.appendChild(canvas); div.appendChild(span); pbrGrid.appendChild(div); });
  }
  const maps = ['basecolor','normal','roughness','metallic','height','ao'];
  for (let i=0;i<maps.length;i++) {
    const canvas = pbrGrid.children[i].querySelector('canvas');
    const blob = await renderPBRMap(128, maps[i]);
    const img = new Image(); img.onload = () => { const ctx = canvas.getContext('2d'); ctx.drawImage(img,0,0,128,128); };
...
patternBar?.addEventListener('touchend', e => { touchEndX = e.changedTouches[0].screenX; if (touchStartX - touchEndX > 50) closeMenu(); }, {passive:true});

// --- Аккордеон на мобильных ---
function initAccordion() { if (window.innerWidth > 860) return; document.querySelectorAll('.accordion-header').forEach(h => { h.addEventListener('click', () => { h.closest('.accordion-group').classList.toggle('open'); renderAll(); }); }); }

// --- Мобильные табы ---
function initMobileTabs() {
  const tabs = document.querySelectorAll('.tab-btn');
  const contents = { texture: document.getElementById('tab-texture'), view3d: document.getElementById('tab-view3d'), pbr: document.getElementById('tab-pbr') };
  ...
  const activateTab = target => {
    ...
    setTimeout(() => { updateSizes(); renderAll(); }, 50);
  };
  tabs.forEach(btn => { btn.addEventListener('click', () => { const target = btn.dataset.tab; activateTab(target); }); });
  if (!document.querySelector('.tab-content.active')) activateTab('texture');
  window.addEventListener('resize', () => { if (window.innerWidth <= 860) { const activeTab = document.querySelector('.tab-btn.active')?.dataset.tab; if (activeTab) activateTab(activeTab); } });
}
...
function initIntegration() {
  ...
  if (btn && select) {
    btn.addEventListener('click', () => {
      ...
      else alert('Временно недоступно');
    });
  }
}
...
function applyPreset(p) {
  ...
  updateUniformsFromUI(); renderAll();
}
savePresetBtn?.addEventListener('click', () => { const p = getCurrentPreset(); const blob = new Blob([JSON.stringify(p)], {type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download=`preset_${Date.now()}.json`; a.click(); });
loadPresetBtn?.addEventListener('click', () => loadPresetInput.click());
loadPresetInput?.addEventListener('change', e => { const f = e.target.files[0]; if(!f) return; const r = new FileReader(); r.onload = ev => { try { applyPreset(JSON.parse(ev.target.result)); } catch(e) { alert('Ошибка'); } }; r.readAsText(f); });

// --- Зум и панорамирование 2D ---
const zoomPanContainer = document.getElementById('zoomPanContainer');
let zoomScale = 1, panX = 0, panY = 0, isPanning2d = false, startPanX=0, startPanY=0;
...
zoomPanContainer?.addEventListener('wheel', e => { e.preventDefault(); zoomScale = Math.min(Math.max(0.5, zoomScale + (e.deltaY>0?-0.1:0.1)),4); updateZoomPan(); }, {passive:false});
zoomPanContainer?.addEventListener('mousedown', e => { isPanning2d=true; startPanX=e.clientX-panX; startPanY=e.clientY-panY; zoomPanContainer.style.cursor='grabbing'; });
window.addEventListener('mousemove', e => { if(!isPanning2d) return; panX=e.clientX-startPanX; panY=e.clientY-startPanY; updateZoomPan(); });
window.addEventListener('mouseup', () => { isPanning2d=false; if(zoomPanContainer) zoomPanContainer.style.cursor='grab'; });

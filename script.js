...
vec3 getColor(float t) { 
  ...
  if (baseIdx >= uColorsCount - 1) { 
    ...
    if (uColorsCount == 3) return uColor2; 
    if (uColorsCount == 4) return uColor3; 
    if (uColorsCount == 5) return uColor4; 
    if (uColorsCount == 6) return uColor5; 
    if (uColorsCount == 7) return uColor6; 
    return uColor7; 
  } 
...
void main() {
  if (uExportMode == 0) {
    ...
    float h = computePattern(vUv); float roughness = 1.0 - pow(h, uRoughnessContrast); roughness = clamp(roughness, 0.04, 0.96); gl_FragColor = vec4(roughness, roughness, roughness, 1.0);
  } else if (uExportMode == 3) { 
    float h = computePattern(vUv); float metallic = clamp((h - uMetalThreshold) * uMetalScale, 0.0, 1.0); gl_FragColor = vec4(metallic, metallic, metallic, 1.0);
  } else if (uExportMode == 4) { 
    float h = computePattern(vUv); gl_FragColor = vec4(h, h, h, 1.0);
...
plane2d = new THREE.Mesh(new THREE.PlaneGeometry(2,2), createMaterial()); 
scene2d.add(plane2d); 
  ...
  activeColors.forEach((col, idx) => {
    ...
    if (activeColors.length > 2) { 
      ...
      removeBtn.addEventListener('click', () => { 
        if (activeColors.length > 2) { activeColors.splice(idx,1); rebuildColorUI(); updateColorUniforms(); updateMaterial(); } 
      }); 
...
document.getElementById('addColorBtn').addEventListener('click', () => { 
  if (activeColors.length < 8) { activeColors.push(new THREE.Color('#FFB347')); rebuildColorUI(); updateColorUniforms(); updateMaterial(); } 
});
rebuildColorUI();
...
['scale','octaves','persistence','lacunarity','saturation','blendMode','rotate','offsetX','offsetY','mirror','warpStrength','warpOctaves','reliefStrength','intensity','tile3dScale','normalStrength','roughnessContrast','metalThreshold','metalScale'].forEach(id => { 
...
function populateSelect(id, items) { 
  ...
  items.forEach(i => { 
    ...
    if(i.v===uniforms.uPatternType.value) o.selected=true; 
    sel.appendChild(o); 
  }); 
...
populateSelect('selectNoise', noisePatterns); 
populateSelect('selectFractal', fractalPatterns); 
populateSelect('selectGradient', gradientPatterns); 
populateSelect('selectGeometric', geometricPatterns);
...
document.getElementById('modelFileInput').addEventListener('change', e => { 
  ...
  }, undefined, () => document.getElementById('modelStatus').textContent='Ошибка загрузки'); 
});
...
document.getElementById('bgImageInput').addEventListener('change', e => { 
  if(e.target.files[0]) { 
    const img = new Image(); 
    img.onload = () => { backgroundImageEl = img; overlayDirty = true; generateOverlayTexture(); }; 
...
function updateLayersUI() { 
  ...
  layers.forEach(layer => { 
    ...
    div.appendChild(thumb); div.appendChild(nameSpan); div.appendChild(delBtn); 
    div.addEventListener('click', (e) => { 
      if(!e.target.closest('button')) { 
        selectedLayerId = layer.id; 
        updateLayersUI(); 
        if(document.getElementById('layerOpacity')) document.getElementById('layerOpacity').value = layer.opacity; 
      } 
    }); 
...
document.getElementById('multiTextureInput').addEventListener('change', async (e) => { 
  if(e.target.files.length) { 
    for(const file of e.target.files) { 
      ...
      layers.push({ id: Date.now()+Math.random(), imgElement: img, name: file.name, x:0.5, y:0.5, scale:0.4, rotation:0, mirror:0, opacity:1.0, syncWithPattern:false, tileX:1, tileY:1 }); 
...
document.getElementById('layerOpacity')?.addEventListener('input', (e) => { 
  if(selectedLayerId) { 
    ...
    if(layer) { 
      ...
      document.getElementById('layerOpacityVal').innerText = layer.opacity.toFixed(2); 
    } 
  } 
});
...
function applyPreset(p) { 
  if(p.colors) { activeColors = p.colors.map(h => new THREE.Color('#'+h)); rebuildColorUI(); updateColorUniforms(); } 
  if(p.uniforms) { 
...
document.getElementById('loadPresetInput').onchange = e => { 
  const f = e.target.files[0]; if(!f) return; 
  const r = new FileReader(); 
  r.onload = ev => { 
    try { applyPreset(JSON.parse(ev.target.result)); } 
    catch(e) { alert('Ошибка загрузки пресета'); } 
  }; 
  r.readAsText(f); 
};
...
document.getElementById('export2DBtn').addEventListener('click', async () => {
  ...
  if(format === 'pbr') {
    ...
    const exportCanvas = document.createElement('canvas'); exportCanvas.width=res; exportCanvas.height=res; exportCanvas.getContext('2d').drawImage(renderer2d.domElement,0, 0,res,res); exportCanvas.toBlob(blob => downloadBlob(blob, `texture.${format}`), format==='jpg'?'image/jpeg':'image/png',0.95); 
  }
});
...
// --- ИСПРАВЛЕНИЕ: Добавлен обработчик клика для аккордеона ---
